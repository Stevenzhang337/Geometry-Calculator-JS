...
var geo = (function() {
    ...
    class Polygon {
        constructor(pointsArr) {
            ...
            let lines = new Array();
            for (let i = 0; i < pointsArr.length; i++){
                let l = new Line(pointsArr[i], pointsArr[(i + 1) % pointsArr.length]);
                lines.push(l);
            }
    ...
    function getPoints(object) {
        ...
        if (isCircle(object)) return [object.center]
    ...
    function getIntersection(object1, object2) {
        if (isLineClass(object1) && isLineClass(object2)) {
            ...
            if (isParallel(object1, object2) && object1 instanceof Line && object2 instanceof Line) {
                if (pointEqual(object1.pt1, object2.pt1) || pointEqual(object1.pt1, object2.pt2)) {
                    return object1.pt1;
                }
                else return object1.pt2;
            }
    ...
    function translate(object, dx, dy) {
        if (isPoint(object)) {
            ...
            let c = new Point(object.center.x + dx, object.center.y + dy);
            return new Circle(c, object.radisu)
        }
        else if (isTriangle(object)) {
            let p1 = new Point(object.pt1.x + dx, object.pt1.y + dy);
            let p2 = new Point(object.pt2.x + dx, object.pt2.y + dy);
    ...
    function getPerpendicularLine(point, line) {
        ...
        if (!isIntersected(line, extLine)) line = lineToExtLine(line);
    ...
    ////////////////////////////////////////////////////////////////////////////////
    //centered at (0,0)
    function newRegPolygon(numOfSides, sideLength, dx, dy) {
        ...
        for (let i = 0; i < pointsArr.length; i++) {
            let cPoint = pointsArr[i];
            cPoint.x += dx;
            cPoint.y += dy;
        }
        if (pointsArr.length == 3) {
            let t = new Triangle(pointsArr[0], pointsArr[1], pointsArr[2]);
    ...
    function newTriangle(congruenceType, measure1, measure2, measure3, dx, dy) {
        if (congruenceType == "SSS") {
            let point1 = new Point(dx,dy);
            let point2 = new Point(measure1 + dx, dy);
            let x = (measure1 ** 2 + measure2 ** 2 - measure3 ** 2) / (2 * measure1);
            let y = (measure2 ** 2 - x ** 2) ** .5
            let point3 = new Point(x + dx, -1 * y + dy);
            return new Triangle(point1, point2, point3);
        }
        else if (congruenceType == "SAS") {
            //law of cosine
            angle = measure2 * Math.PI / 180
            let thirdSideSq = measure1**2 + measure3**2 - 2*measure1*measure2*Math.cos(angle);
            let thirdSide = thirdSideSq ** .5
            return newTriangle("SSS", measure1, measure3, thirdSide, dx, dy);
        }
        else if (congruenceType == "ASA") {
            //law of sine
            angle1 = measure1 * Math.PI / 180;
            angle2 = measure3 * Math.PI / 180
            lastAngle = (Math.PI - angle1 - angle2)
            let side1 = measure2 * Math.sin(angle1) / Math.sin(lastAngle)
            let side2 = measure2 * Math.sin(angle2) / Math.sin(lastAngle)
            return newTriangle("SSS", measure2, side1, side2, dx, dy);
        }
        else if (congruenceType == "AAS") {
            let angle3 = 180 - measure1 - measure2;
            return newTriangle("ASA", measure2, measure3, angle3, dx, dy)
        }
    }
    ...
    function polygonEqual(shape1, shape2) {
        ...
        if ((isTriangle(shape1) || isPolygon(shape1)) && (isTriangle(shape2) || isPolygon(shape2))) {
    ...
    function isIntersected(object1, object2) {
        //line intersection
        if (isLineClass(object1) && isLineClass(object2)) {
            if (isParallel(object1, object2)) {
                if (object1 instanceof ExtLine || object2 instanceof ExtLine) return false
                if (lineEqual(object1, object2)) return false
                //if they intersect at a point
                if (pointEqual(object1.pt1, object2.pt1) || pointEqual(object1.pt1, object2.pt2) || 
                    pointEqual(object1.pt2, object2.pt1) || pointEqual(object1.pt2, object2.pt2)) {
                        return true
                    }
                return false;
            }
            ...
            if (Math.abs(getSlope(object1)) === Infinity || isNaN(getSlope(object1))) {
                ...
                if (object2 instanceof Line) {
                    ...
                    //checks x is satisfied, not injective for y = 0
                    onLine2 = onLine2 && (Math.min(object2.pt1.x, object2.pt2.x) <= x &&
    ...
    return {
        ...
        Triangle: Triangle,
        Polygon: Polygon,
        linEq: (line, x) => linEq(line, x),
        getPoints: (object) => getPoints(object),
        getLines: (object) => getLines(object),
        partition: (line, ratio1, ratio2) => partition(line, ratio1, ratio2),
        getIntersection: (object1, object2) => getIntersection(object1, object2),
        lineToExtLine: (line) => lineToExtLine(line),
        getPerpendicularLine: (point, line) => getPerpendicularLine(point, line),
        getMedian: (point, line) => getMedian(point, line),
        getAngleBisector: (point, line1, line2) => getAngleBisector(point, line1, line2),
        getDiameter: (point, circle) => getDiameter(point, circle),
        getTangent: (point, circle) => getTangent(point, circle),
        newRegPolygon: (numOfSides, length, dx, dy) => newRegPolygon(numOfSides, length, dx, dy),
        newTriangle: (congruenceType, measure1, measure2, measure3, dx, dy) => newTriangle(congruenceType, measure1, measure2, measure3, dx, dy),
        getDistance: (line) => getDistance(line),
        getAngle: (point, line1, line2) => getAngle(point, line1, line2),
        getSides: (object) => getSides(object),
        getIntAngles: (object) => getIntAngles(object),
        getArea: (object) => getArea(object),
        getPerimeter: (object) => getPerimeter(object),
        reflect: (object, reflectLine) => reflect(object, reflectLine),
        rotate: (object, rotatePoint, angle) => rotate(object, rotatePoint, angle),
        translate: (object, dx, dy) => translate(object, dx ,dy),
        dilate: (object, refPoint, scaleF) => dilate(object, refPoint, scaleF),
        pointEqual: (point1, point2) => pointEqual(point1, point2),
        lineEqual: (line1, line2) => lineEqual(line1, line2),
        circleEqual: (circle1, circle2) => circleEqual(circle1, circle2),
        polygonEqual: (polygon1, polygon2) => polygonEqual(polygon1, polygon2),
        getSlope: (line) => getSlope(line),
        isParallel: (line1, line2) => isParallel(line1, line2),
        isCollinear: (point1, point2, point3) => isCollinear(point1, point2, point3),
        isIntersected: (object1, object2) => isIntersected(object1, object2),
        isPerpendicular: (line1, line2) => isPerpendicular(line1, line2),
        onLine: (point, line) => onLine(point, line),
        onCircle: (point, circle) => onCircle(point, circle)
    }
})();