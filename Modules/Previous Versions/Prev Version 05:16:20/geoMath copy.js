...
export {getPoints, getLines, newPoint, partition, getIntersection, getCircumcenter,
        getCentroid, getOrthocenter, getIncenter, newLine, newExtLine, lineToExtLine, 
        getPerpendicularLine, getMedian, getAngleBisector, getDiameter, getTangent,
        newCircle, newPolygon, newRegPolygon, getDistance, getAngle, getSides, 
        getIntAngles, getArea, getPerimeter, reflect, rotate, translate, dilate,
        pointEqual, lineEqual, circleEqual, polygonEqual, getSlope, isParallel, 
        isCollinear, isIntersected, isPerpendicular, onLine, onCircle}
// module.exports = {
...
//     getOrthocenter: getOrthocenter,
//     getIncenter: getIncenter, 

//     //Line Functions
//     newLine: newLine, //Safe
//     newExtLine: newExtLine, //Safe
//     lineToExtLine: lineToExtLine, //Safes

//     getPerpendicularLine: getPerpendicularLine, //Safe
//     getMedian: getMedian, //Safe
//     getAngleBisector: getAngleBisector,

//     getDiamter: getDiameter,
...
//     newRegPolygon: newRegPolygon,

//     //Distance and Angle Functions
//     getDistance: getDistance, //Safe
//     getAngle: getAngle,
//     getSides: getSides,
//     getIntAngles: getIntAngles,
...
//     //Property Functions
//     pointEqual: pointEqual, //Safe
//     lineEqual: lineEqual, //Safe
//     circleEqual: circleEqual,
//     polygonEqual: polygonEqual,

//     getSlope: getSlope, //Safe

//     isParallel: isParallel, //Safe
//     isCollinear: isCollinear,
//     isIntersected: isIntersected, //Safe
//     isPerpendicular: isPerpendicular, //Safe
...
function rotatePointOverPoint(point, rotatePoint, angle) {
    ...
    let r = getDistance(newLine(point, rotatePoint));
...
function isTriangle(triangle) {
    return (triangle instanceof Triangle &&
            ...
            !isCollinear(triangle.pt1, triangle.pt2, triangle.pt3));
}
function isPolygon(polygon) {
    ...
    for (let i = 0; i < polygon.pointsArr.length; i++){
        let l = newLine(polygon.pointsArr[i], polygon.pointsArr[(i + 1) % polygon.pointsArr.length]);
        lines.push(l);
    }
...
function getPoints(object) {
    if (!isLineClass(object) && !isTriangle(object) && !isPolygon(object)) {
        throw new Error("Input must be of type Line/ExtLine or Triangle");
    }
...
function getLines(shape) {
    ...
    if (isTriangle(shape)) {
        let side1 = newLine(shape.pt2, shape.pt3);
        let side2 = newLine(shape.pt1, shape.pt3);
        let side3 = newLine(shape.pt1, shape.pt2);
        return [side1, side2, side3];
    }
    if (isPolygon(shape)) {
        let LineArr = new Array();
        for (let i = 0; i < shape.pointsArr.length; i++){
            let l = newLine(shape.pointsArr[i], shape.pointsArr[(i + 1) % shape.pointsArr.length]);
            LineArr.push(l);
        }
        return LineArr;
    }
...
function newPoint(x,y) {
    if (!isNumber(x) || !isNumber(y)) throw new Error("Inputs must be a Number");
...
function getIntersection(object1, object2) {
    ...
    if ((isCircle(object1) && isLineClass(object2) || (isCircle(object2) && isLineClass(object1)))) {
        ...
        let roots;
        let res;
        if (isVertical(getSlope(object2))){
            roots = invLinEq(object2, 0)
            let i = circleEq(object1, roots);
            res = [newPoint(roots, i[0]), newPoint(roots, i[1])];
        }
        else {
            ...
            res = [newPoint(roots[0], i1), newPoint(roots[1], i2)];
        }
        if (pointEqual(res[0], res[1])) return res[0];
        return res;
    }
    if (isCircle(object1) && isCircle(object2)) {
        if (!isIntersected(object1, object2)) throw new Error("objects do not intersect");
        let commonLine = newLine(object1.center, object2.center)
        ...
        console.log(l);
        l1 = rotate(l, object1.center, angle);
        l2 = rotate(l, object1.center, -1 * angle);
...
function translate(object, dx, dy) {
    if (isPoint(object)) {
        return newPoint(object.x + dx, object.y + dy)
    }
    else if (isLine(object)) {
        let p1 = newPoint(object.pt1.x + dx, object.pt1.y + dy);
        let p2 = newPoint(object.pt2.x + dx, object.pt2.y + dy);
        return newLine(p1, p2);
    }
    else if (isExtLine(object)) {
        let p = newPoint(object.point.x + dx, object.point.y + dy);
        return newExtLine(p, object.slope);

    }
    else if (isTriangle(object)) {
        let p1 = newPoint(object.pt1.x + dx, object.pt1.y + dy);
        let p2 = newPoint(object.pt2.x + dx, object.pt2.y + dy);
        let p3 = newPoint(object.pt3.x + dx, object.pt3.y + dy);
        return newPolygon(p1, p2, p3);
    }
    else if (isPolygon(object)) {
        let arr = new Array();
        for (let i = 0; i < object.pointsArr.length; i++) {
            let p = newPoint(object.pointsArr[i].x + dx, object.pointsArr[i].y + dy);
            arr.push(p);
        }
...
function getCircumcenter(triangle) {
    ...
    return newPoint(x / denom, y / denom);
...
//returns new Line
function newLine(pt1, pt2) {
    if (!isPoint(pt1) || !isPoint(pt2)) throw new Error("Input must be points");
    return new Line(pt1, pt2);
}
//returns a new ExtLine
function newExtLine(point, slope) {
    if (!isPoint(point)) throw new Error("Input must be of type Point");
    if (!isNumber(slope) && !isVertical(slope)) throw new Error("must be valid slope value")
    return new ExtLine(point, slope);
}
...
function dilate(object, refPoint, scaleFactor) {
    ...
    if (isLine(object)) {
        ...
        return newLine(newpt1, newpt2);
    }
    else if (isCircle(object)) {
        let newpt1 = dilatePoint(object.center, refPoint, scaleFactor);
        return newCircle(newpt1, object.center * scaleFactor);
    }
    else if (isTriangle(object)) {
        let newpt1 = dilatePoint(object.pt1, refPoint, scaleFactor);
        let newpt2 = dilatePoint(object.pt2, refPoint, scaleFactor);
        let newpt3 = dilatePoint(object.pt3, refPoint, scaleFactor);
        return newPolygon(newpt1, newpt2, newpt3);
    }
    else if (isPolygon(object)) {
...
function getAngleBisector(point, line1, line2) {
    ...
    let oppositeSide = newLine(outerPoint1, outerPoint2);
    ...
    let angleBi = newLine(point, scaledLine.pt2);
...
function getDiameter(point, circle) {
    ...
    let radius = newLine(point, circle.center);
...
function getTangent(point, circle) {
    ...
    return newExtLine(point, -1 / slope);
}
...
function newCircle(center, radius) {
    ...
    if (!isCircle(c)) throw new Error("Invalid Circle");
    return c;
}
...
function newRegPolygon(numOfSides, sideLength) {
    ...
    let circle = newCircle(newPoint(0,0), r);
...
function getSides(object) {
    ...
    if (isTriangle(object)) {
        let side1 = getDistance(newLine(object.pt2, object.pt3));
        let side2 = getDistance(newLine(object.pt1, object.pt3));
        let side3 = getDistance(newLine(object.pt1, object.pt2));
        return [side1, side2, side3];
    }
    if (isPolygon(object)) {
        let arr = new Array();
        for (let i = 0; i < object.pointsArr.length; i++) {
            let l = newLine(object.pointsArr[i], object.pointsArr[(i + 1) % object.pointsArr.length]);
...
function getArea(shape) {
    ...
    if (isPolygon(shape)) {
        ...
        for (let i = 1; i < ptsArr.length - 1; i++) {
            let t = newPolygon(ptsArr[0], ptsArr[i], ptsArr[i+1])
...
function lineEqual(line1, line2) {
    ...
    if (line1 instanceof ExtLine && line2 instanceof ExtLine) {
        let ln = newLine(line1.point, line2.point);
        if (isVertical(line1) && isVertical(line2) && isVertical(getSlope(ln))) return true;
        return (line1.slope == line2.slope && getSlope(ln) == line1.slope);
    }
...
function circleEqual(circle1, circle2) {
    ...
    return (pointEqua(circle1.center, circle2.center) && circle1.radius == circle2.radius);
}
...
function getSlope(line) {
    ...
    if (line instanceof Line) {
        dy = (line.pt1.y - line.pt2.y);
        dx = (line.pt1.x - line.pt2.x);
        let m = dy / dx;
...
function isIntersected(object1, object2) {
    //line intersection
    if (isLineClass(object1) && isLineClass(object1)) {
        if (isParallel(object1, object2)) return false;
        //ensures line1 is vertical if at all; checks vertical lines once for both lines
        if (Math.abs(getSlope(object1)) === Infinity || isNaN(getSlope(object2))) {
            let lineTemp = object2;
            object2 = object1;
            object1 = lineTemp;
        }
    ...
    //circle intersection
        ...
        let d = getDistance(newLine(object1.center, object2.center));
...
function isCollinear(point1, point2, point3) {
    ...
    let line1 = newLine(point1, point2);
    let line2 = newLine(point2, point3);
    let line3 = newLine(point1, point3);
...
function onLine(point, line) {
    ...
    if (line instanceof Line) {
        if (Math.min(line.pt1.x, line.pt2.x) <= point.x && point.x <= Math.max(line.pt1.x, line.pt2.x)) {
            return almostEqual(linEq(line, point.x), point.y);
        }
        return false;
    }
    if (line instanceof ExtLine) {
        let tempLine = newLine(point, line.point);
...
function onCircle(point, circle) {
    if (!isPoint(point)) throw new Error("Input must be of type Point");
    if (!isCircle(circle)) throw new Error("Input must be of type Circle");
    let line = newLine(point, circle.center);
